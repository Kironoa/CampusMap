...
class ArView(
    ...
    private suspend fun buildModelNode(nodeData: Map<String, Any>): ModelNode? {
        ...
        // Column-major 4x4 matrix: position in [12,13,14], rotation about Y (yaw)
        // in column 2 (m[8], m[10]), scale = length of column 0 (rotation-independent).
        val tx = transformation[12].toFloat()
        val ty = transformation[13].toFloat()
        val tz = transformation[14].toFloat()
        val sx = kotlin.math.sqrt(
            (transformation[0] * transformation[0] +
                transformation[1] * transformation[1] +
                transformation[2] * transformation[2]).toFloat()
        )
        val yaw = kotlin.math.atan2(
            transformation[8].toFloat(),
            transformation[10].toFloat()
        )
        // Node.rotation is euler angles in degrees.
        val yawDegrees = (yaw * 180.0f / kotlin.math.PI.toFloat())
        ...
        return try {
            sceneView.modelLoader.loadModelInstance(fileLocation)?.let { modelInstance ->
                object : ModelNode(
                    ...
                    position = ScenePosition(tx, ty, tz)
                    rotation = Rotation(x = 0f, y = yawDegrees, z = 0f)
                }
            } ?: run {
                null
            }
