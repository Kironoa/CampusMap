        ...
        return Stack(
          children: [
            FlutterMap(
              ...
              children: [
                TileLayer(
                  urlTemplate: _mapDark
                      ? 'https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png'
                      : 'https://tile.openstreetmap.org/{z}/{x}/{y}.png',
                  subdomains: _mapDark ? ['a', 'b', 'c'] : [],
                ...
                onPressed: () {
                  showModalBottomSheet(
                    ...
                    builder: (_) => AINavSheet(
                      onNavigate: (result) {
                        Navigator.pop(context);
                        if (result.floor == 'ground' || result.floor == 'second') {
                          Navigator.pushNamed(context, '/floor_plan').then((_) {
                            if (result.targetRoomId != null) {
                              // Floor plan will handle room highlighting via args or state
                            }
                          });
                        } else if (result.targetLandmarkId != null) {
                          final landmark = tcgcLandmarks.firstWhere(
                            (l) => l.id == result.targetLandmarkId,
                            orElse: () => tcgcLandmarks.first,
                          );
